// server.js (Tüm Rotaları İçeren Final Sürüm)
...
app.post('/login', (req, res) => {
    ...
    db.serialize(() => {
        db.get("SELECT first_used_at FROM access_keys WHERE key = ?", [userKey], (err, row) => {
            if (err) { res.render('login', { error: 'Bir veritabanı hatası oluştu.' }); db.close(); return; }
            if (row) {
                req.session.isAdmin = false;
                if (row.first_used_at === null) {
                    const now = new Date().toISOString();
                    db.run("UPDATE access_keys SET first_used_at = ? WHERE key = ?", [now, userKey], (updateErr) => {
                        if (updateErr) { res.render('login', { error: 'Veritabanı güncellenirken bir hata oluştu.' }); }
                        else { req.session.isLoggedIn = true; res.redirect('/viewer'); }
                        db.close();
                    });
                } else {
            ...
            } else { res.render('login', { error: 'Geçersiz anahtar.' }); db.close(); }
        });
    });
});
...
app.get('/viewer', async (req, res) => {
    ...
    db.get("SELECT setting_value FROM settings WHERE setting_key = 'copy_text'", [], async (err, row) => {
        if (err) { res.status(500).send("Ayar okuma hatası"); db.close(); return; }
        const settingsFromDb = {};
        // Bu kısım normalde tüm ayarları çekmeli ama şimdilik sadece copy_text'i alalım
        settingsFromDb.copy_text = row ? row.setting_value : '';

        // Tüm ayarları yeniden yükleyerek güncel hali alalım
        loadSettings(async () => {
...
app.post('/update-copy-text', (req, res) => {
    ...
    db.run("UPDATE settings SET setting_value = ? WHERE setting_key = 'copy_text'", [newText], (err) => {
        if (err) { res.status(500).send("Ayar kaydetme hatası"); } else {
...
app.post('/update-azure-settings', (req, res) => {
    if (!req.session.isLoggedIn || !req.session.isAdmin) { return res.status(403).send("Yetkiniz yok."); }
    ...
    stmt.finalize((err) => {
        if (err) {
            res.status(500).send("Azure ayarları kaydedilemedi.");
            db.close();
            return;
        }
